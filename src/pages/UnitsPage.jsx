...
import { BookOpen, ChevronRight, File } from "lucide-react"; // استيراد أيقونة السهم
...
export default function UnitsPage() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col items-center justify-center p-4 sm:p-8 relative ">
      <AnimatedBackground />
      <AnimatedCharacter />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-w-3xl w-full relative z-10">
        {units.map((unit) => (
          ...
          >
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center text-white shadow-md" style={{ backgroundColor: unit.color }}>
            ...
            {openUnitId === unit.id && (
              ...
              >
                {lessons.map((lesson) => (
                  <div key={lesson.id} className="relative flex flex-row">
                    <button
                      onClick={(e) => handleLessonClick(e, lesson.id)}
                      className={`
    w-full py-3 px-4 rounded-xl text-left flex justify-between items-center
    transition-all duration-300
    ${openLessonId === lesson.id
                          ? 'bg-purple-200 text-purple-900 font-semibold shadow-inner'
                        ...
                        {lessonContent.map((content) => {
                          ...
                          return (
                            <button
                              ...
                              className="flex items-center gap-3 py-2 px-4 rounded-l hover:bg-purple-100 text-left text-gray-700 border-l-2 border-purple-300"
                            >
